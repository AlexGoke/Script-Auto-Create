...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io-f-014-056
case title: 基础IO-SAS-SSD-单一IO满单条带-顺序混合读写
test category: SAS-SSD-基础IO
check point: 基础IO-SAS-SSD-单一IO满单条带-顺序混合读写
...
@steps: 1、组建符合条件的VD后进行快速初始化
        ...
        读写比例rdpct=90，xfersize=384K测试并发混合读写，
        3、进行数据一致性校验
        4、清理环境
...
class BasicioSasSsdSequentialReadWrite(BasicioMultiVDScriptBase):
    ...
    def set_parameters(cls):
        ...
        # 测试数据读写比例设置
        cls.vdbench_parameters_dict['rdpct'] = '90'
        # 测试数据随即比例设置
        cls.vdbench_parameters_dict['seekpct'] = '0'
        # 测试数据块大小及分配设置
        cls.vdbench_parameters_dict['xfersize'] = '384k'
        # vdbench一致性校验
