...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io-f-014-055
case title: 基础IO-SAS-SSD-条带非对齐模式-随机混合读写
test category: SAS-SSD-基础IO
check point: 基础IO-SAS-SSD-条带非对齐模式-随机混合读写
...
@steps: 1、组建符合条件的VD后进行快速初始化
        ...
        随机比例seekpct=50，读写比例rdpct=50，xfersize=（383K，1535K，2680K，
        9200K）测试并发随机混合读写，
        3、进行数据一致性校验
        4、清理环境
...
class BasicioSasSsdRandomReadWrite(BasicioMultiVDScriptBase):
...
def main() -> None:
    BasicioSasSsdRandomReadWrite.run()
