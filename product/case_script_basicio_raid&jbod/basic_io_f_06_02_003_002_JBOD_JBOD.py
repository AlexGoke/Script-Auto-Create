...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io_f-06-02-003-002
case title: 基础IO-JBOD-JBOD-大IO-随机写
test category: 两个JBOD并行IO
check point: JBOD-JBOD并行随机写正常
...
读写比例rdpct=0，xfersize=（913K，4096K，10M，256M）测试并发随机写，
3、进行数据一致性校验
4、清理环境
...
class BasicioJbodJbodRandomWrite(BasicioMultipleObjectScriptBase):
...
def main() -> None:
    BasicioJbodJbodRandomWrite.run()
