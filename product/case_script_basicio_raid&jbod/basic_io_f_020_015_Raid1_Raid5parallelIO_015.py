...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io-f-020-015
case title: 基础IO-Raid1-Raid5-并行大小IO混合-随机混合读写
test category: 两个VD并行IO
check point: 基础IO-Raid1-Raid5-并行大小IO混合-随机混合读写
...
@steps: 1、组建符合条件的VD后进行快速初始化
        ...
        读写比例rdpct=40，xfersize=（2K，64K，128M，256M）测试并发随机混合读写，
        3、进行数据一致性校验
        4、清理环境
...
class BasicioRaid1Raid5parallelRandomReadWrite(BasicioMultipleRaidScriptBase):
...
def main() -> None:
    BasicioRaid1Raid5parallelRandomReadWrite.run()
