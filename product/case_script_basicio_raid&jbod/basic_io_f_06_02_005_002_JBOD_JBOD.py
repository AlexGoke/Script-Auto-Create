...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io_f-06-02-005-002
case title: 基础IO-JBOD-JBOD-大IO-随机混合读写
test category: 两个JBOD并行IO
check point: JBOD-JBOD并行随机混合读写正常
...
读写比例rdpct=50，xfersize=（913K，4096K，10M，256M）测试并发随机混合读写，
3、进行数据一致性校验
4、清理环境
...
class BasicioJbodJbodRandomReadWrite(BasicioMultipleObjectScriptBase):
...
def main() -> None:
    BasicioJbodJbodRandomReadWrite.run()
