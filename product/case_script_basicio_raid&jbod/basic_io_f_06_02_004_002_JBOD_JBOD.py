...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io_f-06-02-004-002
case title: 基础IO-JBOD-JBOD-大IO-顺序写
test category: 两个JBOD并行IO
check point: JBOD-JBOD并行顺序写正常
...
读写比例rdpct=0，xfersize=（913K，4096K，10M，256M）测试并发顺序写，
3、进行数据一致性校验
4、清理环境
...
class BasicioJbodJbodSequenceWrite(BasicioMultipleObjectScriptBase):
    ...
    def set_parameters(cls) -> None:
        ...
        cls.vdb_elapsed = '120'
        # vdbench数据块大小
        cls.vdb_xfersize = '(913K,25,4096K,25,10M,25,256M,25)'
        # 读写比例
        cls.vdb_rdpct = '0'
        # 随机率
        cls.vdb_seekpct = '0'
...
def main() -> None:
    BasicioJbodJbodSequenceWrite.run()
