...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io-f-020-017
case title: 基础IO-Raid1-Raid5-并行大IO-顺序混合读写
test category: 两个VD并行IO
check point: 基础IO-Raid1-Raid5-并行大IO-顺序混合读写
...
@steps: 1、组建符合条件的VD后进行快速初始化
        ...
        读写比例rdpct=50，xfersize=（1024K，4096K，10M，256M）测试并发顺序混合读写，
        3、进行数据一致性校验
        4、清理环境
...
class BasicioRaid1Raid5ParallelSequentialReadWrite(BasicioMultipleRaidScriptBase):
...
def main() -> None:
    BasicioRaid1Raid5ParallelSequentialReadWrite.run()
