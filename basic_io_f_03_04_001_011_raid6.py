...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io_f-03-04-002-011
case title: 基础IO-raid6-条带非对齐模式-顺序读
test category: Raid6
...
@steps:
1、组建符合条件的VD后进行快速初始化
2、进行IO的vdbench配置：对齐align = 127K  测试时间 elapse=5min，IO并发thread=32，
随机比例seekpct=0，读写比例rdpct=100，xfersize=（127K，1025K，4090K，8200K）测试并发顺序读，
3、清理环境
@changelog
"""
...
class BasicioRaid6SequenceRead(BasicioMultiVDScriptBase):
    ...
    def set_parameters(cls) -> None:
        ...
        # vdbench数据块大小
        cls.vdbench_parameters_dict[constants.VDB_XFERSIZE] = '(127K,25,1025K,25,4090K,25,8200K,25)'
        # 读写比例
        cls.vdbench_parameters_dict[constants.VDB_RDPCT] = '100'
        # LBA地址对齐
        cls.vdbench_parameters_dict[constants.VDB_ALIGN] = '127K'
        # 随机率
