...
# -*- coding:utf-8 -*-
"""
case number: ps3-v1.0.0-basic_io-f-014-065
case title: 基础IO-SAS-SSD-条带对齐模式-顺序混合读写
test category: SAS-SSD-基础IO
check point: 基础IO-SAS-SSD-条带对齐模式-顺序混合读写
...
@steps: 1、组建符合条件的VD后进行快速初始化
        2、进行IO的vdbench配置：对齐align = 384K  测试时间 elapse=2min，IO并发thread=32，
        随机比例seekpct=0，读写比例rdpct=50，xfersize=（384K，1536K，2687K，
        9216K）测试并发顺序混合读写，
        3、进行数据一致性校验
        4、清理环境
...
class BasicioSasSsdSequentialReadWrite(BasicioMultiVDScriptBase):
    ...
    def set_parameters(cls):
        ...
        # 测试数据块大小及分配设置
        cls.vdbench_parameters_dict['xfersize'] = '(384K,25,1536K,25,2687K,25,9216K,25)'
        # vdbench一致性校验
        cls.vdbench_parameters_dict['consistency_check'] = False
        # 偏移量
        cls.vdbench_parameters_dict['offset'] = None
        # 对齐
        cls.vdbench_parameters_dict['align'] = '384K'
        # 测试区间
